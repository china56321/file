...
def w_non_max_suppression(prediction, num_classes, conf_thres=0.5, nms_thres=0.6):
    # 求左上角和右下角
...
def scale_coords(img1_shape, coords, img0_shape, ratio_pad=None):
    ...
    if ratio_pad is None:  # calculate from img0_shape
        gain_h = (img1_shape[0]) / (img0_shape[0])  # gain  = old / new
        gain_w = (img1_shape[1]) / (img0_shape[1])  # gain  = old / new

        pad = (img1_shape[1] - img0_shape[1] * gain_w) / 2, (img1_shape[0] - img0_shape[0] * gain_h) / 2  # wh padding
    else:
        gain = ratio_pad[0][0]
        pad = ratio_pad[1]
...
def detect_onnx(official=True, image_path=None):
    ...
    session = onnxruntime.InferenceSession('last.onnx')
    ...
    # resized = letterbox_image(image_src, (img_size_h, img_size_w))
    img_in = np.transpose(resized, (2, 0, 1)).astype(np.float32)  # HWC -> CHW
    # img_in = np.transpose(resized, (2, 1, 0)).astype(np.float32)  # HWC -> CHW
    img_in = np.expand_dims(img_in, axis=0)
    img_in /= 255.0
    print(img_in)
    print("Shape of the  input image: ", img_in.shape)
    ...
    if official and len(outputs) == 4:   # model.model[-1].export = boolean ---> True:3 False:4
        ...
        for index, out in enumerate(outputs):
            ...
            print('stride_w_h=',feature_w,feature_h,stride_w,stride_h)
...
def display(detections=None, image_path=None, line_thickness=None, text_bg_alpha=0.0):
    ...
    # test_img=cv2.resize(img,(640,320))
    ...
    boxs[:, :] = scale_coords((320, 640), boxs[:, :], (h, w)).round()
...
if __name__ == '__main__':
    ...
    image_path = './inference/images/3.jpg'
    with torch.no_grad():
